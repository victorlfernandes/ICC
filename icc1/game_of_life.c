...
int main(){
    ...
    for (int i=0; i<linhas; i++){
        //precisamos alocar 'colunas+1' espacos por causa do \n 
        //presente no final de cada linha
...
void calcula_geracoes(char **matriz, int linhas, int colunas, char tipo_vizinhanca){
    ...
    //e fazendo as mudancas necessarias em cada celula da matriz 'aux'
    for (int i=0; i<linhas; i++){
        for (int j=0; j<colunas; j++){

            //determinando os 8 vizinhos da celula atual
            char vizinhos[8];
            ...
            //condicao para a celula morrer
            if ((matriz[i][j] == 'x') && (n_vizinhos_vivos < 2 || n_vizinhos_vivos > 3)){
                aux[i][j] = '.';
            }
            //condicao para a celula nascer
